...
struct Options {
    ...
    Options() :
      background(true), port(8080), user(""), command("/bin/bash"), device(""), authname("none"),
          charset("ascii"), diff(true), max_sessions(20), max_http_connections(0), local_only(false),
          name("anytermd") {
    }
};
