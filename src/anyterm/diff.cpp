...
namespace DiffAlgo {
  ...
  class Differ {
    ...
    private:
      ...
      typedef ::std::vector< int > V_impl_t;
      V_impl_t V_impl;
      int& V(int k) {
        return V_impl[V_offset + k];
      }

      typedef ::std::vector< V_impl_t > stored_V_impls_t;
    ...
    public:
      ...
      Differ(const SEQ& a, const SEQ& b, typename fragment_seq< SEQ >::Type& r, bool s = true, int md = -1) :
        A(a), B(b), N(A.size()), M(B.size()), max_D((md == -1) ? (M + N) : (::std::min(md, M + N))),
            store(s), result(r), V_size(::std::max(2 * (max_D) + 1, 2)), // 2 allows for d=0 V(1) special case
  ...
  void make_trivial_solution(const ::std::string& A, const ::std::string& B, string_fragment_seq& result) {
  ...
  void string_diff(const ::std::string& A, const ::std::string& B, string_fragment_seq& result) {
    ...
    try {
      ...
      // equal N.)
      if ((sz * sz) < max_mem) {
        Differ< ::std::string > d1(A, B, result, true, max_time);
        d1.solve();
        d1.find_trace();
        return;
      }
      ...
      Differ< ::std::string > d2(A, B, result, false, max_time);
      d2.solve();

      // We could now solve this with space complexity O(ND), if that
      // were acceptable:
      if (sz * d2.get_edit_distance() < max_mem) {
        Differ< ::std::string > d3(A, B, result, true, d2.get_edit_distance());
        d3.solve();
        d3.find_trace();
        return;
      }
    ...
    catch (Differ< ::std::string >::max_D_exceeded) {
      make_trivial_solution(A, B, result);
    }
  }
