...
Terminal::Terminal(int rows, int cols, Screen& screen_) :
  screen(screen_), rows_(rows), cols_(cols), saved_cursor_row(0), saved_cursor_col(0), scrolltop(0),
      scrollbottom(rows - 1), charset_mode(0), crlf_mode(false), dirty(true), esc_state(normal),
  ...
  for (int i = 0; i < nparams; ++i) {
    switch (params[i]) {
      ...
      case 1:
        ...
        break;
      case 2:
        current_attrs.halfbright = true;
        break;
      case 4:
        current_attrs.underlined = true;
        break;
      case 5:
      ...
      case 22:
        current_attrs.halfbright = false;
        break; // are "set normal intensity".
      case 24:
        current_attrs.underlined = false;
        break;
      case 25:
      ...
      case 39:
        ...
        break;
      case 49:
        current_attrs.background = 0;
        break;

      default:
        if (params[i] >= 30 && params[i] <= 37) {
          current_attrs.foreground = params[i] - 30;
        } else if (params[i] >= 40 && params[i] <= 47) {
          current_attrs.background = params[i] - 40;
        } else {
          if (debug_emulation) {
