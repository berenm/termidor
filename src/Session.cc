...
Session::Session(int r,
                 ...
                 string charset_,
                 bool diff_) :
  rows(r), cols(c), scrollback(sb), time_out(t), charset(charset_), diff(diff_), utf8_to_charset("UTF-8",
                                                                                                 charset),
      charset_to_ucs4(charset, UCS4_NATIVE), ucs4_to_utf8(UCS4_NATIVE, "UTF-8"), screen(rows,
                                                                                        cols,
                                                                                        scrollback),
      error(false), term(rows, cols, screen), activity(af(boost::bind(&Session::process_output, this, _1),
