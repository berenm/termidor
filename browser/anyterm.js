...
function send() {
  ...
  send_loader.load(url_prefix + "anyterm-module", "a=send&s=" + session + cachebust() + "&k=" + encodeURIComponent(kb_buf));
  kb_buf = "";
}
...
var Anyterm = {
  ...
  substitute_variables: function(string) {
    ...
    return string.replace(/%v/g, version).replace(/%h/g, hostname);
  }
}
...
function create_term(elem_id, title, rows, cols, p, charset, scrollback) {
  if (open) {
    console.error("Terminal is already open");
    return;
  }
